...
DBZCCG.Interface.addMenus = function(networkObject, elem) {
    console.log('Adding All Menus.');

    $(document).ready(function() {
        DBZCCG.Interface.addLeftSideMenu(networkObject, elem);
    });
};

DBZCCG.Interface.addLeftSideMenu = function(networkObject, elem) {
    ...
    var leftMenuContent = '<div id="left-sticky-menu" class="left-sticky-menu-left">\
                        <div class="btn-group-vertical">\
                            <button data-container="body" data-title="Communication Box" type="button" class="btn btn-default btn-sm input-sm" id="btn-show-communication-box">\
                                <span class="glyphicon glyphicon-comment"></span><span class="badge"></span>\
                            </button>\
    ...
    $('#btn-show-communication-box').click(function() {
        ...
        if ($(this).hasClass('active')) {
            $('#btn-show-communication-box').children('.badge').text('');
    ...
    DBZCCG.Interface.addCommunicationBox(elem, networkObject);

    /* End of communication Box */

};

DBZCCG.Interface.addCommunicationBox = function(elem, networkObject) {
                        ...
                        <div class="row">\
                            <div class="chat-input col-md-12 input-group">\
                                ...
                                <div class="input-group-btn">\
                                    <button type="button" id="chat-submit" class="chat-send-button form-control btn btn-default">\
                                        <span>Send</span>\
                                    </button>\
                                </div>\
                            </div>\
    ...
    var HTMLContent = '\
            <div id="communication-box">\
                <ul class="nav nav-tabs">\
                    ...
                    <li id="#log-box-tab" style="display: none;"><a href="#log-box" data-toggle="tab">Log</a></li>\
                </ul>\
    ...
    networkObject.subscribe('/topic/chat', function(output) {
        var message = JSON.parse(output.body);
        var appendableMessage = message.time + " <b>" + message.sourceUser.displayName + "</b>: " + $('<div>').html(message.message).text();
        
        if(!$('#communication-box').is(":visible")) {
            var badges = $('#btn-show-communication-box').children('.badge').text() || 0;
            badges++;
            $('#btn-show-communication-box').children('.badge').html(badges);
        }
        
        $('#chat-entries').append("<div>" + appendableMessage + "</div>");
        $('#chat-entries').animate({scrollTop: $('#chat-entries')[0].scrollHeight}, 'slow');
    });

    networkObject.subscribe('/user/topic/chat', function(output) {
        $('#chat-entries').animate({scrollTop: $('#chat-entries')[0].scrollHeight}, 'slow');
         var message = JSON.parse(output.body);
    });
...
//
        ...
        $('#chat-submit')
            ...
            if ($('#chat-input').val().match(/\w+/g) !== null) {
                networkObject.send('/app/ws/chat/global', {}, $('#chat-input').val());
                $('#chat-input').val('');
            }
        });
...
$(document).ready(function() {
    $('body').append('\
<div class="modal in" id="dbz-interface-select-modal" role="dialog" data-backdrop="static" data-keyboard="false" aria-labelledby="Select" aria-hidden="true">\
